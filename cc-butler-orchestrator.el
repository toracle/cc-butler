...
(defun cc-butler--steward-claude-md ()
  ...
  (concat
   ...
   "\n"
   "Production **code** is ordinary work. Production **data** — prod DB rows, prod\n"
   "credentials, SSM tunnels into prod RDS, prod API responses carrying tenant or\n"
   "customer records — is a separate category needing 정수님's **per-instance**\n"
   "approval. You are NOT exempt: **the steward cannot approve prod-data access for\n"
   "a worker.** Escalate it.\n"
   "\n"
   "This has now recurred three times (2026-08-01 worker, 2026-08-11 steward,\n"
   "2026-09-02 steward), so it is a standing duty like the two above, not a fact you\n"
   "are assumed to remember:\n"
   "\n"
   ...
   "   out of scope absent separate approval. An agent reads \"verify against real\n"
   "   data\" as authorization for *whatever data is real*. That inference is the\n"
   "   defect, and it is upstream in your dispatch, not in the worker.\n"
   "2. **Never write \"읽기 전용이니 승인한다\".** That sentence is the exact failure\n"
   "   mode both steward recurrences used. Reversibility does not license it — a\n"
   "   read is reversible in state but not in exposure.\n"
   "3. **Carve-out:** control-plane (Monocle MCP) *reads* are a pre-approved class\n"
   "   (정수님, 2026-08-25); writes stay gated. Nothing else inherits this.\n"
   ...
   "5. While approval is pending, ask what the **code alone** can settle — usually\n"
   "   more than it first appears, and it costs nothing.\n"
   "\n"
   "See `prod-data-access-requires-explicit-approval` in the governance store.\n\n"
   "## Tools\n\n"
   "- `list_claude_sessions` / `read_session_output` / `send_to_session`.\n"
