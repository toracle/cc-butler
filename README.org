...
(use-package cc-butler
  :vc (:url "git@github.com:toracle/cc-butler.git" :rev :newest)
...
(unless (package-installed-p 'cc-butler)
  (package-vc-install "git@github.com:toracle/cc-butler.git"))
...
* After a merge: syncing a running installation

~cc-butler~ installs are ~package-vc~-managed — a pinned git checkout, not a
registry package. *Merging a fix to ~main~ on GitHub does not reach any
running instance by itself.* Each machine hosting a fleet must explicitly
upgrade and recompile, and a live Emacs process keeps executing the old code
until it does. Never treat "merged" as "deployed" — verify by observation.

On each machine:

1. Upgrade the install:
   #+begin_src emacs-lisp
   (package-vc-upgrade (car (alist-get 'cc-butler package-alist)))
   #+end_src
   (or interactively: ~M-x package-vc-upgrade RET cc-butler RET~)

2. Verify the *files on disk* actually moved — don't infer this from the
   command not erroring:
   #+begin_src sh
   git -C ~/.emacs.d/elpa/cc-butler log -1 --oneline
   #+end_src
   Compare that SHA against the expected merge commit / ~origin/main~. Only a
   match confirms the install itself is current.

3. If a live Emacs session was already running cc-butler before step 1, its
   already-loaded function definitions stay on the *old* code in memory even
   after the files on disk are updated. Restart Emacs, or explicitly reload
   the changed file(s) (~M-x load-file~), so the running process actually
   executes the new code. Step 2's SHA only tells you the disk is current —
   it does not tell you the running process picked it up; check that
   separately (e.g. inspect the live behavior, or ~symbol-file~ / re-eval the
   function and compare against the source).
...
That script's load-path is deliberately ordered so an elpa-installed copy of
cc-butler doesn't shadow the working tree under test — see its own header
comment for why that matters.

** Known issue: broken system ~emacs~ on some machines

On at least one machine running these tests (confirmed 2026-08-15), the
~emacs~ found on ~PATH~ fails outright:

#+begin_src sh
$ emacs --batch --eval '(princ emacs-version)'
exec: /opt/homebrew/Cellar/emacs-plus@30/30.2/Emacs.app/Contents/MacOS/Emacs: No such file or directory
#+end_src

It is a Homebrew shim exec-ing an app-bundle path that no longer exists on
that machine. *Run the same command above to check your own machine* — if it
fails the same way, use the working binary confirmed for this repo's tests
(Emacs 30.2) directly instead of ~emacs~:

#+begin_src sh
/Applications/Emacs.app/Contents/MacOS/Emacs -Q --batch -l tests/run-tests.el
#+end_src

The root cause has not been investigated (a moved or reinstalled app bundle
is a guess, not a confirmed cause). Scope: this only affects CLI batch runs
(tests, this troubleshooting section) — a live/GUI Emacs process already
running cc-butler is unaffected. This is a fact about *that one machine*,
not about the repo or a universal cc-butler requirement; a machine with a
working ~emacs~ on ~PATH~ has nothing to do here.
