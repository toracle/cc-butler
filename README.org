...
Run ~M-x cc-butler-doctor~ (~?~ in the session list) any time to check these —
...
| ~g~     | refresh                                             |
| ~?~     | ~cc-butler-doctor~ — check for fresh-install gaps   |
...
Once running, the worker firehose (nudges + ~report_to_butler~) routes to the
